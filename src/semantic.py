...
class Semantic:
    ...
    def findLiterals(self):
        for entry in self.symbol_table:
            if entry["identifier"].split("[")[0] == self.identifier_value:
                ...
                return
            print(f"a{self.literal_value}/{entry["identifier"].split("[")[0]}/{self.identifier_value}")
    ...
    def process_strcElement(self):        
        if self.lookahead == "id":
            ...
            inStruct = self.isDeclaredInParent(self.identifier_value, scope)
            print(f"process strcElement: {self.identifier_value} {scope}")
            if not inStruct:
