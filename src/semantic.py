...
class Semantic:
    ...
    def semantic_process(self, lookahead, current_token_index):
        ...
        if self.lookahead in relate1_op:
            ...
            if leftLogic == False and rightLogic == False:
    ...
    def handle_declaration(self):
        ...
        print(f"Declaration {self.type_value}")
        ...
        #Initialization
            ...
            if self.type_value != "const":
                self.findType()
            if self.type_value == "segm" and token[self.current_token_index+1] == "(":
    ...
    def handle_identifier(self): 
        if self.is_segment: 
            ...
            temp = self.current_token_index+1
            if temp < len(token):
                if token[temp] == "(":
                    self.type_value = ""
                    self.segmID = lexeme[self.current_token_index]
                    self.is_functionCall = True
            if self.type_value == "array":
